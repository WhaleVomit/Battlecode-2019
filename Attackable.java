...
public class Attackable extends Movable {
    ...
    public Action2 closeToEnemy(Robot2 R) {
        if (Z.euclidDist(R) > 100) return moveToward(R.x,R.y);
        return moveTowardNaive(R.x,R.y);
    }

    public Action2 preacherPosition(Robot2 R) {
        if (Z.euclidDist(R) > 49 || R.unit == PROPHET || Z.lastTurn[R.y][R.x] <= Z.CUR.turn-2) return closeToEnemy(R);
        return minimizeDamage(R);
    }

    public Action2 crusaderPosition(Robot2 R) {
        // if (Z.euclidDist(R) > 64 && R.unit == PROPHET && totDamageAfter(Z.CUR.x,Z.CUR.y) == 0 && !attacking(R)) 
        if (Z.euclidDist(R) > 100) return closeToEnemy(R);
        ...
        if (Z.euclidDist(R) > 49 || R.unit == PROPHET) return closeToEnemy(R);
        return minimizeDamage(R);
    }
