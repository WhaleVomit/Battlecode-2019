...
public class secretMap {
  ...
  public int dist(int x, int y) { return dist[y][x]; }
  public int dist(int t) { return t == MOD ? MOD : dist(Z.fdiv(t,64),t%64); }
  public int next(int x, int y) { return next[y][x]; }
  public int next(int t) { return t == MOD ? MOD : next(Z.fdiv(t,64),t%64); }

  public int close(int x, int y) { return close[y][x]; }
  public int close(int x) { return x == MOD ? MOD : close(Z.fdiv(x,64),x%64); }
  public int distClose(int x, int y) { return dist(close(x,y)); }
  public int distClose(int x) { return x == MOD ? MOD : distClose(Z.fdiv(x,64),x%64); }

  public int closestUnseen() {
  ...
  public int closestUnused(boolean[][] B) {
  ...
  public int closestChurch(boolean ourteam) { return closest(ourteam ? Z.myChurch : Z.otherChurch); }
  public int closestCastle(boolean ourteam) { return closest(ourteam ? Z.myCastle : Z.otherCastle); }
  public int closestStruct(boolean ourteam) {
  ...
  public int distHome() { return distClose(closestStruct(true)); }

  public Action2 move(int x, int y) {
  ...
  public Action2 move(int t) { return t == MOD ? null : move(Z.fdiv(t,64),t%64); }
  public Action2 moveFake(int x, int y) {
    int t = next(close(x,y)); if (t == MOD) return null;
    int dx = Z.fdiv(t,64)-Z.CUR.x, dy = (t%64)-Z.CUR.y;
    // Z.log("WHAT "+Z.coordinates(t)+" "+Z.CUR.coordinates()+" "+x+" "+y+" "+dx+" "+dy);
    return Z.moveAction(dx,dy);
  }
  public Action2 moveFake(int t) { return t == MOD ? null : moveFake(Z.fdiv(t,64),t%64); }

  public Action2 moveUnseen() { return move(closestUnseen()); }
  public Action2 moveYourStruct() {
    int x = closestStruct(true);
    if (x == MOD) return moveUnseen();
    return move(x);
  }
  public Action2 moveEnemyStruct() {
    int x = closestStruct(false);
    if (x == MOD) return moveUnseen();
    return move(x);
  }
  public Action2 moveCloseSparse() {
  ...
  public Action2 moveReallySparse() {
